...
public class UserAction extends BaseAction {
	...
	//安卓端登陆界面
	public String andrLogin() throws Exception {
		boolean isLogin;
		ServletOutputStream out = response.getOutputStream();
		isLogin = us.checkUser(u);			
		JSONObject json=new JSONObject();
		json.put("userId", u.getUId());
		json.put("state", isLogin?"YES":"NO");
		out.write(json.toString().getBytes("utf-8"));
		return null;
	}

	//安卓修改用户密码
	public String andrEdit() throws Exception {		
//		us.updateUser(u);	
		boolean isEdit;
		ServletOutputStream out = response.getOutputStream();
		//判断传入的对象是否为空
		isEdit = us.updateUser(u,3);	
		JSONObject json = new JSONObject();			
		json.put("state", isEdit?"YES":"NO");
		out.write(json.toString().getBytes("utf-8"));
		return null;
	}
